...
#MATERIAL PROPERTIES 
    ...
    #GEOMETRY DEFINITION#
    #In rectangular_mesh(base,height,number of vertical divisions,number of horizontal divisions,thickness)
    Nodes_Coord,Element_Connect,Nodes_Vn,Nodes_Thickness=Mesh_Rect_Cyl.rectangular_mesh(8,8,20,20,1)
    ...
    #NOTE: this BCs apply to a 16x16 mesh. If the mesh changes, change to the corresponding nodes. 
    bcdof=BC(1,[0,1,2],bcdof)
    BC_nodes.append(1)
    
    bcdof=BC(421,[0,1,2],bcdof)
    BC_nodes.append(273)
    
    bcdof=BC(441,[0,1,2],bcdof)
    BC_nodes.append(289)
    
    #Set all the inner BCs
    for i in range(Nodes_Coord.shape[0]):
        if i != 0 or i != 272 or i != 297:
            bcdof=BC(i+1,[0,1],bcdof)
            BC_nodes.append(i+1)
    ...
    R,F_nodes,F_components=Force_Rect_Mesh([8,8,0],-2,5,Nodes_Coord,R,
